...
class DataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor eval sdpo_loss", logger=logger)
    def compute_sdpo_pg_loss(self, data: DataProto) -> dict[str, Any]:
        """Compute SDPO (self-distillation) policy loss on a batch without updating weights.

        This is intended for validation/test logging. It mirrors the SDPO branch inside `update_policy`,
        but runs in eval/no_grad mode and returns a scalar loss.
        """
        self.actor_module.eval()
        if self.teacher_module is not None and self.teacher_module is not self.actor_module:
            self.teacher_module.eval()

        self_distillation_cfg = self.config.get("self_distillation", None)
        loss_mode = self.config.policy_loss.loss_mode
        if self_distillation_cfg is None or loss_mode != "sdpo":
            raise ValueError("compute_sdpo_pg_loss is only valid when policy_loss.loss_mode == 'sdpo'.")

        pad_token_id = data.meta_info.get("pad_token_id", None)
        if pad_token_id is None:
            raise ValueError("pad_token_id must be provided in data.meta_info for compute_sdpo_pg_loss.")

        temperature = data.meta_info.get("temperature", 1.0)

        # Keep only what we need for the forward.
        select_keys = [
            "input_ids",
            "attention_mask",
            "position_ids",
        ...
        data = data.select(batch_keys=select_keys, non_tensor_batch_keys=["multi_modal_inputs"] if "multi_modal_inputs" in data.non_tensor_batch else [])

        # Split into micro-batches for memory safety (reuse training micro-batch size).
        micro_bs = self.config.ppo_micro_batch_size_per_gpu
        if micro_bs is None:
            raise ValueError("ppo_micro_batch_size_per_gpu must be set for compute_sdpo_pg_loss.")
        ...
        with torch.no_grad():
            for micro_batch in micro_batches:
                ...
                teacher_out = self._forward_micro_batch(
                    ...
                    return_all_logps=return_all_logps,
                    distill_topk=distill_topk,
                    topk_indices=student_out.get("topk_indices"),
                    module=teacher_model,
                )
                ...
                # For eval logging, we do not apply IS ratio (set old_log_probs=student so ratio=1).
                old_log_probs = student_log_prob.detach()

                pg_loss, _ = compute_self_distillation_loss(
                    student_log_probs=student_log_prob,
                    teacher_log_probs=teacher_log_prob,
                    response_mask=response_mask,
                    self_distillation_config=self_distillation_cfg,
                    old_log_probs=old_log_probs,
                    student_all_log_probs=student_out.get("all_logps"),
                    teacher_all_log_probs=teacher_out.get("all_logps"),
                    student_topk_log_probs=student_out.get("topk_logps"),
                    teacher_topk_log_probs=teacher_out.get("topk_logps"),
                    self_distillation_mask=None,
                    loss_agg_mode=self.config.loss_agg_mode,
                    rollout_is_weights=None,
                )

                total_loss += float(pg_loss.detach().item())
                denom += 1

        return {"actor/sdpo_pg_loss": total_loss / max(1, denom)}
