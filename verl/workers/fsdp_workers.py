...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            actor_optimizer = build_optimizer(
                actor_module_fsdp.parameters(), optim_config, module=actor_module_fsdp
            )
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_eval_sdpo_loss")
    def compute_sdpo_pg_loss(self, data: DataProto):
        """Compute SDPO policy loss on a batch (no update).

        Used for validation/test-time logging.
        """
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics = self.actor.compute_sdpo_pg_loss(data=data)
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        critic_optimizer = build_optimizer(critic_module.parameters(), config.optim, module=critic_module)
