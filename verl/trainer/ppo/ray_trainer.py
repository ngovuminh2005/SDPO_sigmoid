...
class RayPPOTrainer:
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            test_batch.meta_info.setdefault("pad_token_id", self.tokenizer.pad_token_id)
            test_batch.meta_info.setdefault("temperature", self.config.actor_rollout_ref.rollout.temperature)

            # Compute SDPO loss on validation generations (no update) if supported by worker.
            # This logs a scalar comparable to train-time actor/pg_loss.
            try:
                sdpo_out = self.actor_rollout_wg.compute_sdpo_pg_loss(test_batch)
                sdpo_metrics = sdpo_out.meta_info.get("metrics", {}) if sdpo_out is not None else {}
                if "actor/sdpo_pg_loss" in sdpo_metrics:
                    val = sdpo_metrics["actor/sdpo_pg_loss"]
                    # Some worker-group paths may return list-valued metrics; reduce to scalar.
                    if isinstance(val, list):
                        val = float(np.mean(val)) if len(val) > 0 else float("nan")
                    else:
                        val = float(val)
                    sdpo_loss_vals.append(val)
                else:
                    print(
                        f"[validate] compute_sdpo_pg_loss returned no actor/sdpo_pg_loss. keys={list(sdpo_metrics.keys())}"
                    )
            except Exception as e:
                # Keep validation robust even if worker doesn't implement SDPO loss, but surface the error.
                print(f"[validate] compute_sdpo_pg_loss failed: {type(e).__name__}: {e}")
        ...
        if len(sdpo_loss_vals) > 0:
            metric_dict[f"val-aux/{data_sources[0]}/sdpo_pg_loss"] = float(np.mean(sdpo_loss_vals))
        return metric_dict
    ...
    def _train_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        """Same as `_val_metrics_update`, but prefixes metrics with train-core/train-aux."""
        data_src2var2metric2val = process_validation_metrics(data_sources, sample_uids, reward_extra_infos_dict)
        metric_dict = {}
        for data_source, var2metric2val in data_src2var2metric2val.items():
            core_var = "acc" if "acc" in var2metric2val else "reward"
        ...
        if len(sample_turns) > 0:
            sample_turns = np.concatenate(sample_turns)
            metric_dict["train-aux/num_turns/min"] = sample_turns.min()
            metric_dict["train-aux/num_turns/max"] = sample_turns.max()
            metric_dict["train-aux/num_turns/mean"] = sample_turns.mean()

        return metric_dict

    def _eval_acc_on_batch(self, batch: DataProto, *, n: int, reward_fn) -> dict[str, Any]:
        """Evaluate acc/reward metrics@N on a single batch (train batch)."""
        data_source_lst = []
        reward_extra_infos_dict: dict[str, list] = defaultdict(list)
        sample_turns = []
        sample_uids = []
        ...
        # Repeat like validation to get @n
        test_batch = batch.repeat(repeat_times=n, interleave=True)
        test_gen_batch = self._get_gen_batch(test_batch)
        test_gen_batch.meta_info = {
            "eos_token_id": self.tokenizer.eos_token_id,
            "pad_token_id": self.tokenizer.pad_token_id,
        ...
        # compute reward/acc via reward function (same path as validation)
        result = self._compute_or_extract_reward(test_batch, reward_fn=reward_fn, return_dict=True)
        reward_tensor = result["reward_tensor"]
        scores = reward_tensor.sum(-1).cpu().tolist()
        reward_extra_infos_dict["reward"].extend(scores)
        reward_extra_infos_dict["score"].extend(scores)
        ...
        data_sources = np.concatenate(data_source_lst, axis=0)
        return self._train_metrics_update(data_sources, sample_uids, reward_extra_infos_dict, sample_turns)
    ...
    def _save_checkpoint(self):
        ...
        self._push_checkpoint_to_hf(local_global_step_folder)

    def _sanitize_hf_repo_id(self, value):
        ...
        if value.count("/") > 1:
            value = value.replace("/", "-")
        if len(value) > 96:
            value = f"{value[:72].rstrip('-.')}-{value[-23:].lstrip('-.')}"
        return value or "checkpoint-model"

    def _push_checkpoint_to_hf(self, checkpoint_folder):
        if not self.config.trainer.get("hf_push_enabled", False):
            return

        from huggingface_hub import HfApi

        model_name = os.path.basename(str(self.config.actor_rollout_ref.model.path).rstrip("/"))
        checkpoint_name = f"global_step_{self.global_steps}"
        data_path = os.environ.get("DATA_PATH", os.environ.get("TASK", "dataset")).rstrip("/")
        dataset_name = os.path.basename(data_path) or "dataset"
        optim_name = self.config.actor_rollout_ref.actor.optim.optimizer
        repo_id = self._sanitize_hf_repo_id(f"{model_name}-{checkpoint_name}-{dataset_name}-{optim_name}")
        token = os.environ.get("HF_TOKEN")
        if not token:
            raise RuntimeError("HF push is enabled but HF_TOKEN is not set.")

        api = HfApi(token=token)
        if "/" not in repo_id:
            repo_id = f"minhnv7/{repo_id}"
        ...
        api.upload_folder(
            ...
            repo_type="model",
            token=token,
            commit_message=f"Upload checkpoint global_step_{self.global_steps}",
        )
        print(f"Uploaded checkpoint global_step_{self.global_steps} to Hugging Face: {repo_id}")
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                train_eval_batch = deepcopy(batch)
                ...
                if (
                    ...
                    with marked_timer("testing", timing_raw, color="green"):
                        # Train-set acc@N on the current train batch (for monitoring; can be expensive).
                        try:
                            train_acc_metrics = self._eval_acc_on_batch(
                                train_eval_batch,
                                n=self.config.actor_rollout_ref.rollout.val_kwargs.n,
                                reward_fn=self.reward_fn,
                            )
                            metrics.update(train_acc_metrics)
                        except Exception as e:
                            print(f"[train-eval] failed: {type(e).__name__}: {e}")
