...
class Tracking:
    ...
    _TIME_METRIC_STEP_KEY = "time_metrics/cumulative_training_time_s"
    _TIME_METRIC_KEYS = {
        "train-core/sciknoweval/acc/mean@16": "time_metrics/train-core/sciknoweval/acc/mean@16",
        "val-core/sciknoweval/acc/mean@16": "time_metrics/val-core/sciknoweval/acc/mean@16",
        "actor/pg_loss": "time_metrics/actor/pg_loss",
    }
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None, group_name=None):
        ...
        self.logger = {}
        self._cumulative_training_time_s = 0.0
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            self._define_wandb_time_metrics(wandb)
        ...
        if "vemlp_wandb" in default_backend:
            ...
            self._define_wandb_time_metrics(vemlp_wandb)
    ...
    def log(self, data, step, backend=None):
        time_metrics = {}
        if ("wandb" in self.logger or "vemlp_wandb" in self.logger) and isinstance(data, dict):
            time_metrics = self._compute_time_metrics(data)

        for default_backend, logger_instance in self.logger.items():
            if backend is None or default_backend in backend:
                data_for_logger = data
                if default_backend in {"wandb", "vemlp_wandb"} and time_metrics:
                    data_for_logger = {**data, **time_metrics}
                logger_instance.log(data=data_for_logger, step=step)

    def _define_wandb_time_metrics(self, wandb):
        wandb.define_metric(self._TIME_METRIC_STEP_KEY)
        for time_metric_key in self._TIME_METRIC_KEYS.values():
            wandb.define_metric(time_metric_key, step_metric=self._TIME_METRIC_STEP_KEY)
        wandb.define_metric("time_metrics/sdpo_pg_loss", step_metric=self._TIME_METRIC_STEP_KEY)

    @staticmethod
    def _is_numeric_scalar(value: Any) -> bool:
        return isinstance(value, Number) and not isinstance(value, bool)

    def _compute_time_metrics(self, data: dict[str, Any]) -> dict[str, float]:
        timing_step = data.get("timing_s/step")
        if not self._is_numeric_scalar(timing_step):
            return {}

        self._cumulative_training_time_s += float(timing_step)
        time_metrics = {self._TIME_METRIC_STEP_KEY: self._cumulative_training_time_s}

        for source_metric_key, time_metric_key in self._TIME_METRIC_KEYS.items():
            metric_val = data.get(source_metric_key)
            if self._is_numeric_scalar(metric_val):
                time_metrics[time_metric_key] = float(metric_val)

        sdpo_pg_loss = data.get("sdpo_pg_loss")
        if not self._is_numeric_scalar(sdpo_pg_loss):
            sdpo_pg_loss = data.get("actor/sdpo_pg_loss")
        if not self._is_numeric_scalar(sdpo_pg_loss):
            for key, val in data.items():
                if key.endswith("/sdpo_pg_loss") and self._is_numeric_scalar(val):
                    sdpo_pg_loss = val
                    break
        if self._is_numeric_scalar(sdpo_pg_loss):
            time_metrics["time_metrics/sdpo_pg_loss"] = float(sdpo_pg_loss)
